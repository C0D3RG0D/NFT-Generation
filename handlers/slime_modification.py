    ...
    for w in range(WIDTH):
        for h in range(HEIGHT):
            ...
            if pixel != (0, 0, 0, 0):
                ...
                # Красим только объект динозавра
                slime_pixels[w, h] = (
                    pixel[0] + r_byte,
                    pixel[1] + g_byte,
                    pixel[2] + b_byte,
...
def create_image(image_name):
    ...
    # Генерируем текст и редактируем динозавра
    ...
    # Добавляем характеристики каждого динозавра
    ...
    if not os.path.exists("result/all_dino"):
        os.mkdir("result/all_dino")
    img.save(f"result/all_dino/{image_name}({str(character_rarity).replace('.', ',')}).png")
